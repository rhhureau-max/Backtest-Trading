...
Analyzes 5-minute trading data files from 2018 onwards

FVG Quality Definition:
- Good FVG: Price doesn't return to FVG zone after 5 candles (by 9:05)
- Bad FVG: Price returns to FVG zone after 5 candles (by 9:05)
"""
...
def analyze_fvg_quality_at_830(df, year):
    """
    Analyze FVG quality at 8:30 AM for a given year (5-minute candles)
    ...
    for idx in indices_830:
        ...
        # Get the three candles for FVG detection (8:25, 8:30, 8:35)
        candle_825 = df.loc[idx - 1]  # Previous (should be 8:25)
        candle_830 = df.loc[idx]      # Current (8:30)
        candle_835 = df.loc[idx + 1]  # Next (should be 8:35)
        
        # Validate adjacent candles
        expected_prev_time = time(8, 25)
        expected_next_time = time(8, 35)
        
        prev_time = candle_825['Column2']
        if isinstance(prev_time, str):
            if prev_time not in ['08:25:00', '8:25:00', '08:25', '8:25']:
                continue
        elif prev_time != expected_prev_time:
            continue
        
        next_time = candle_835['Column2']
        if isinstance(next_time, str):
            if next_time not in ['08:35:00', '8:35:00', '08:35', '8:35']:
                continue
        elif next_time != expected_next_time:
            continue
        
        # Extract OHLC values (Column3=Open, Column4=High, Column5=Low, Column6=Close)
        high_825 = float(candle_825['Column4'])
        low_825 = float(candle_825['Column5'])
        high_835 = float(candle_835['Column4'])
        low_835 = float(candle_835['Column5'])
        
        # Check for FVG
        is_bullish_fvg = low_835 > high_825
        is_bearish_fvg = high_835 < low_825
        
        if not is_bullish_fvg and not is_bearish_fvg:
            continue
        
        # Get next 5 candles AFTER the third candle closes (8:40 to 9:05)
        ...
        if is_bullish_fvg:
            # Bullish FVG zone: between high_825 and low_835
            fvg_low = high_825
            fvg_high = low_835
            ...
            # Bearish FVG zone: between low_825 and high_835
            fvg_low = high_835
            fvg_high = low_825
...
def main():
    ...
    print("=" * 80)
    print("FVG QUALITY ANALYSIS AT 8:30 AM - 5-MINUTE DATA (2018-2025)")
    print("=" * 80)
    print()
    print("FVG Quality Definition:")
    print("  - Good FVG: Price doesn't return to FVG zone after 5 candles (by 9:05)")
    print("  - Bad FVG: Price returns to FVG zone after 5 candles (by 9:05)")
    print()
    print("=" * 80)
    print()
    ...
    if results:
        print("=" * 80)
        print("SUMMARY - FVG QUALITY AT 8:30 AM (5-MINUTE DATA)")
        print("=" * 80)
        print()
        ...
        with open('fvg_quality_analysis_5m.txt', 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("FVG QUALITY ANALYSIS AT 8:30 AM - 5-MINUTE DATA (2018-2025)\n")
            f.write("=" * 80 + "\n\n")
            f.write(f"Analysis Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            
            f.write("FVG Quality Definition:\n")
            f.write("  - Good FVG: Price doesn't return to FVG zone after 5 candles (by 9:05)\n")
            f.write("  - Bad FVG: Price returns to FVG zone after 5 candles (by 9:05)\n\n")
            
            f.write("SUMMARY - FVG QUALITY AT 8:30 AM (5-MINUTE DATA)\n")
            f.write("=" * 80 + "\n\n")
        ...
        print(f"Results saved to 'fvg_quality_analysis_5m.txt'")
    else:
