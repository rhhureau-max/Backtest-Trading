#!/usr/bin/env python3
"""
Script to run 70/30 partial exit strategy (NO TIME STOP) backtest on all years and generate detailed results.
"""

import sys
import os
from datetime import datetime
from judas_swing_fvg_70_30_no_timestop_backtest import run_backtest
...
def main():
    """Generate detailed results for 70/30 partial exit strategy (NO TIME STOP)."""
    
    # Store all results
    all_results = {}
    
    print("="*80)
    print("GENERATING 70/30 PARTIAL EXIT STRATEGY (NO TIME STOP) RESULTS FOR ALL YEARS")
    print("="*80)
    print()
    ...
    print("\n✓ Detailed results saved to: RESULTATS_70_30_NO_TIMESTOP.md")
    print()
...
def generate_markdown(all_results):
    ...
    # Header
    md_content.append("# Résultats - Stratégie Judas Swing + FVG (70/30 Partial Exit - NO TIME STOP)")
    md_content.append("")
    ...
    md_content.append("")
    md_content.append("### Gestion du Risque - 70/30 PARTIAL EXIT (NO TIME STOP)")
    ...
    md_content.append("   - Si pas atteint: sortie au breakeven")
    md_content.append("")
    md_content.append("### ⚠️ PAS DE TIME STOP")
    md_content.append("")
    md_content.append("**Différence clé avec la version standard:**")
    md_content.append("- **PAS de sortie à 12:00 Chicago**")
    md_content.append("- Les trades continuent jusqu'à atteindre TP1, TP2, SL, ou breakeven")
    md_content.append("- Permet aux trades de se développer complètement")
    md_content.append("- Maximise le potentiel de profit sur les mouvements lents")
    md_content.append("- Peut augmenter l'exposition au risque overnight (mais breakeven après TP1)")
    md_content.append("")
    md_content.append("---")
    md_content.append("")
    ...
    md_content.append("")
    md_content.append("### Performance Globale 70/30 (NO TIME STOP)")
    md_content.append("")
    ...
    md_content.append("### Avantages de la Stratégie 70/30 Sans Time Stop")
    md_content.append("")
    md_content.append("1. **Développement Complet:** Les trades peuvent se développer entièrement sans contrainte de temps")
    md_content.append("2. **Sécurisation Rapide Optimisée:** 70% du profit sécurisé rapidement à l'équilibre")
    md_content.append("3. **Protection Maximale:** SL au breakeven après TP1 = zéro risque sur 30% restants")
    md_content.append("4. **Pas de Sortie Prématurée:** Évite de fermer des trades gagnants avant qu'ils atteignent leur potentiel")
    md_content.append("5. **Mouvements Lents:** Capture les mouvements qui prennent plus de temps à se développer")
    md_content.append("6. **Protection Breakeven:** Même sans time stop, le risque est éliminé après TP1")
    md_content.append("")
    
    md_content.append("### Comparaison avec Version Time Stop (12:00)")
    md_content.append("")
    md_content.append("**Avantages NO TIME STOP:**")
    md_content.append("- Plus de trades atteignent TP2 (30% restants)")
    md_content.append("- Pas de fermeture forcée des positions profitables")
    md_content.append("- Meilleur pour les marchés avec mouvements lents/étendus")
    md_content.append("")
    md_content.append("**Inconvénients potentiels:**")
    md_content.append("- Exposition plus longue (mais protégée par breakeven)")
    md_content.append("- Peut tenir des positions overnight")
    md_content.append("- Nécessite plus de patience")
    md_content.append("")
    
    md_content.append("### Comparaison avec Autres Stratégies")
    md_content.append("")
    md_content.append("**vs 70/30 avec Time Stop:**")
    md_content.append("- Potentiel de profit supérieur sur TP2")
    md_content.append("- Pas de fermeture forcée à 12:00")
    md_content.append("- Même protection après TP1 (breakeven)")
    md_content.append("")
    ...
    md_content.append("- Pas de limite de temps pour atteindre TP2")
    md_content.append("")
    
    md_content.append("### Recommandations")
    md_content.append("")
    md_content.append("1. **Idéal pour marchés tendanciels:** Laisse les trades se développer complètement")
    md_content.append("2. **Traders patients:** Nécessite d'accepter que les trades puissent durer plus longtemps")
    md_content.append("3. **Maximisation du potentiel:** Excellent pour capturer les grands mouvements")
    md_content.append("4. **Gestion émotionnelle:** 70% sécurisé + breakeven = stress minimal")
    md_content.append("5. **Protection optimale:** Zéro risque après TP1 grâce au breakeven")
    md_content.append("")
    md_content.append("---")
    md_content.append("")
    ...
    md_content.append("- **NO TIME STOP:** Les trades continuent jusqu'à TP ou SL, pas de fermeture à 12:00")
    md_content.append("")
    md_content.append("---")
    md_content.append("")
    md_content.append("*Rapport généré automatiquement par le script judas_swing_fvg_70_30_no_timestop_backtest.py*")
    
    # Write to file
    with open("RESULTATS_70_30_NO_TIMESTOP.md", "w", encoding="utf-8") as f:
        f.write("\n".join(md_content))
