#!/usr/bin/env python3
"""
Fair Value Gap (FVG) Backtest Strategy - NY Opening with Size Filtering
=========================================================================
This script backtests an FVG strategy with size filtering.
Tests FVGs < 15 points and < 10 points separately for 1R, 1.5R, and 2R targets.

Strategy Rules:
- Identify ALL FVGs between 08:30-09:00 NY time
- Filter by FVG size (< 15 points, < 10 points)
...
class NYOpeningFVGBacktest:
    """Backtest engine for NY Opening FVG strategy with size filtering."""
    
    def __init__(self, data_directory: str, rr_level: float = 1.0, max_fvg_size: float = None):
        """
        ...
        Args:
            ...
            max_fvg_size: Maximum FVG size in points (e.g., 15.0, 10.0)
        """
        self.data_directory = data_directory
        self.rr_level = rr_level
        self.max_fvg_size = max_fvg_size
    ...
    def detect_fvg(self, candle_1: pd.Series, candle_2: pd.Series, candle_3: pd.Series) -> Optional[Dict]:
        ...
        if candle_1['High'] < candle_3['Low']:
            ...
            if self.max_fvg_size is not None and gap_size >= self.max_fvg_size:
                return None
    ...
    def run_backtest(self) -> List[Dict]:
        ...
        size_filter_text = f"< {self.max_fvg_size} pts" if self.max_fvg_size else "ALL sizes"
        print(f"\n🔍 Running backtest with {self.rr_level}R take profit level...")
        print(f"   FVG Size Filter: {size_filter_text}")
    ...
    def calculate_statistics(self) -> Dict:
        ...
        stats = {
            'rr_level': self.rr_level,
            'max_fvg_size': self.max_fvg_size,
    ...
    def display_results(self):
        ...
        print("\n" + "="*80)
        print(f"BACKTEST RESULTS - {self.rr_level}R TP, FVG Size {size_filter_text}")
        print("="*80)
...
def run_filtered_backtest():
    """Run backtest for filtered FVG sizes and all RR levels."""
    
    data_dir = "/home/runner/work/Backtest-Trading/Backtest-Trading"
    rr_levels = [1.0, 1.5, 2.0]
    fvg_size_filters = [15.0, 10.0]  # Max FVG sizes to test
    
    print("\n" + "="*80)
    print("NY OPENING FVG BACKTEST - FILTERED BY SIZE")
    print("Strategy: Fair Value Gap Breakout (NY Session 08:30-09:00)")
    print("Testing FVGs with size filters: < 15 points, < 10 points")
    print("="*80)
    ...
    for size_filter in fvg_size_filters:
        ...
        for rr in rr_levels:
            ...
            print(f"\n\n{'='*80}")
            print(f"RUNNING BACKTEST: FVG < {size_filter} pts, {rr}R TAKE PROFIT")
            print('='*80)
            
            backtest = NYOpeningFVGBacktest(data_dir, rr_level=rr, max_fvg_size=size_filter)
            ...
            if trades:
                ...
                output_file = os.path.join(data_dir, f'ny_opening_fvg_filtered_{size_filter}pts_{rr}R_results.csv')
                df_results.to_csv(output_file, index=False)
                print(f"\n✅ Results saved to: {output_file}")
            else:
                print(f"\n⚠️ No trades for FVG < {size_filter} pts, {rr}R level")
    
    # Display comparative summary for each size filter
    for size_filter in fvg_size_filters:
        print("\n\n" + "="*80)
        print(f"COMPARATIVE SUMMARY - FVG SIZE < {size_filter} POINTS")
        print("="*80)
        ...
        if all_stats[size_filter]:
            ...
            metrics = [
                ...
                ('Avg FVG Size (pts)', 'avg_fvg_size', '.2f'),
            ]
            ...
            print(f"\n🏆 BEST PERFORMERS (FVG < {size_filter} pts)")
            print("-" * 80)
    ...
    print("-" * 80)
    print(f"{'Filter':<20} {'RR':<10} {'Trades':>10} {'Win Rate':>12} {'Total PnL':>12} {'PF':>8}")
    print("-" * 80)
    ...
    for size_filter in fvg_size_filters:
        if all_stats[size_filter]:
            ...
            print(f"< {size_filter} pts{' '*(20-len(str(size_filter))-5)} {best_rr}R{' '*(10-len(str(best_rr))-1)} "
