...
def load_nq_data(base_path, timeframe):
    ...
    for file in sorted(files):
        try:
            ...
            # Convert to timezone-aware CST, then to Paris time
            ...
            # Extract Paris components
            df['Date_Paris'] = df['DateTime_Paris'].dt.date
            df['Time_Paris'] = df['DateTime_Paris'].dt.time
            df['Hour_Paris'] = df['DateTime_Paris'].dt.hour
            df['DayOfWeek'] = df['DateTime_Paris'].dt.dayofweek  # 0=Monday, 6=Sunday
    ...
    combined = combined.sort_values('DateTime_Paris').reset_index(drop=True)
    
    print(f"Loaded {len(combined)} records for {timeframe}")
    print(f"Date range: {combined['DateTime_Paris'].min()} to {combined['DateTime_Paris'].max()}")
...
def analyze_judas_swing(df_15m):
    ...
    Asian Session: 00:00-07:00 Paris time
    Judas Window: 07:30-09:00 Paris time
    ...
    # Group by date
    df_15m['Date_Paris'] = pd.to_datetime(df_15m['Date_Paris'])
    
    for date, day_data in df_15m.groupby('Date_Paris'):
        # Skip weekends
        if day_data['DayOfWeek'].iloc[0] >= 5:
            continue
        
        # Asian session: 00:00-07:00 Paris time
        asian_session = day_data[(day_data['Hour_Paris'] >= 0) & (day_data['Hour_Paris'] < 7)]
        ...
        judas_window = day_data[
            (day_data['Hour_Paris'] == 7) & (day_data['DateTime_Paris'].dt.minute >= 30) |
            (day_data['Hour_Paris'] == 8)
        ]
        
        if len(judas_window) == 0:
            continue
        
        # London session: 08:00-13:00 Paris time for success validation
        london_session = day_data[(day_data['Hour_Paris'] >= 8) & (day_data['Hour_Paris'] < 13)]
...
def analyze_ny_midnight(df_15m):
    ...
    NY Midnight: 05:00 Paris time (winter) or 06:00 Paris time (summer/DST)
    Entry time: 08:00 Paris time (London open)
    ...
    for date, day_data in df_15m.groupby('Date_Paris'):
        ...
        # Find NY Midnight (05:00 or 06:00 Paris time)
        midnight_candidates = day_data[
            ((day_data['Hour_Paris'] == 5) | (day_data['Hour_Paris'] == 6)) &
            (day_data['DateTime_Paris'].dt.minute == 0)
        ]
        ...
        # Find 08:00 Paris time entry point
        ...
        # London session for validation (08:00-13:00 Paris time)
        london_session = day_data[(day_data['Hour_Paris'] >= 8) & (day_data['Hour_Paris'] < 13)]
        ...
        # Bullish bias: price at 08:00 > midnight level
        ...
        # Bearish bias: price at 08:00 < midnight level
...
def analyze_h4_structure(df_4h, df_1d):
    ...
    df_4h['Date_Paris'] = pd.to_datetime(df_4h['Date_Paris'])
    df_1d['Date_Paris'] = pd.to_datetime(df_1d['Date_Paris'])
    ...
    for i in range(window_size, len(df_4h) - 5):
        ...
        current_date = df_4h.iloc[i]['Date_Paris']
        d1_candle = df_1d[df_1d['Date_Paris'] == current_date]
...
def analyze_smt_divergence(df_nq_15m, df_es_15m):
    ...
    df_nq_15m['Date_Paris'] = pd.to_datetime(df_nq_15m['Date_Paris'])
    df_es_15m['Date_Paris'] = pd.to_datetime(df_es_15m['Date_Paris'])
    
    # Merge on datetime for comparison
    merged = pd.merge(
        df_nq_15m[['DateTime_Paris', 'Date_Paris', 'High', 'Low', 'Close', 'Hour_Paris', 'DayOfWeek']],
        df_es_15m[['DateTime_Paris', 'High', 'Low', 'Close']],
        on='DateTime_Paris',
        suffixes=('_NQ', '_ES'),
        how='inner'
    )
    
    for date, day_data in merged.groupby('Date_Paris'):
        # Skip weekends
        if day_data['DayOfWeek'].iloc[0] >= 5:
            continue
        
        # SMT window: 06:00-08:00 Paris time
        smt_window = day_data[(day_data['Hour_Paris'] >= 6) & (day_data['Hour_Paris'] < 8)]
        ...
        prev_window = day_data[(day_data['Hour_Paris'] >= 4) & (day_data['Hour_Paris'] < 6)]
        ...
        # London session for validation (08:00-13:00 Paris time)
        london = day_data[(day_data['Hour_Paris'] >= 8) & (day_data['Hour_Paris'] < 13)]
...
def analyze_d1_london_correlation(df_1d, df_15m):
    ...
    London session (08:00+ Paris time) is bullish?
    """
    print("\n" + "="*70)
    ...
    for i in range(len(df_1d) - 1):
        ...
        if close_position > 0.75:
            ...
            next_date = df_1d.iloc[i + 1]['Date_Paris']
            london_data = df_15m[
                (df_15m['Date_Paris'] == next_date) &
                (df_15m['Hour_Paris'] >= 8) &
                (df_15m['Hour_Paris'] < 13)
            ]
...
def analyze_asian_range_sweep(df_15m):
    ...
    Asian Range: 00:00-07:00 Paris time
    Sweep Window: 07:00-09:00 Paris time
    """
    print("\n" + "="*70)
    ...
    for date, day_data in df_15m.groupby('Date_Paris'):
        ...
        # Asian session
        asian = day_data[(day_data['Hour_Paris'] >= 0) & (day_data['Hour_Paris'] < 7)]
        ...
        # Sweep window
        sweep = day_data[
            ((day_data['Hour_Paris'] == 7) | (day_data['Hour_Paris'] == 8))
        ]
        ...
        rest = day_data[day_data['Hour_Paris'] >= 9]
...
def analyze_volume_volatility(df_15m):
    """
    Identify the exact Paris time hour when volume significantly increases.
    """
    print("\n" + "="*70)
    print("VOLUME & VOLATILITY ANALYSIS")
    print("="*70)
    
    # Calculate average volume by hour (Paris time)
    hourly_volume = df_15m.groupby('Hour_Paris')['Volume'].agg(['mean', 'median', 'std'])
    hourly_volume = hourly_volume.sort_values('mean', ascending=False)
    
    print("\nAverage Volume by Hour (Paris time) - Top 10:")
    print("-" * 50)
    for hour, row in hourly_volume.head(10).iterrows():
        print(f"Hour {hour:02d}:00 Paris - Mean: {row['mean']:,.0f}, Median: {row['median']:,.0f}")
    
    # Calculate volatility (range) by hour
    df_15m['Range'] = df_15m['High'] - df_15m['Low']
    hourly_volatility = df_15m.groupby('Hour_Paris')['Range'].agg(['mean', 'median'])
    hourly_volatility = hourly_volatility.sort_values('mean', ascending=False)
    
    print("\nAverage Volatility (Range) by Hour (Paris time) - Top 10:")
    print("-" * 50)
    for hour, row in hourly_volatility.head(10).iterrows():
        print(f"Hour {hour:02d}:00 Paris - Mean: {row['mean']:.2f}, Median: {row['median']:.2f}")
    
    # Identify early morning calm (typically 05:00-07:00 Paris time)
    early_morning = df_15m[(df_15m['Hour_Paris'] >= 5) & (df_15m['Hour_Paris'] < 7)]
    avg_early_morning_vol = early_morning['Volume'].mean()
    
    print(f"\nEarly Morning (05:00-07:00 Paris) Average Volume: {avg_early_morning_vol:,.0f}")
    
    # Find when volume increases significantly
    print("\nHours with >50% higher volume than early morning:")
    print("-" * 50)
    for hour, row in hourly_volume.iterrows():
        if row['mean'] > avg_early_morning_vol * 1.5:
            increase = (row['mean'] / avg_early_morning_vol - 1) * 100
            print(f"Hour {hour:02d}:00 Paris - {increase:.0f}% higher volume")
    
    return {
        'hourly_volume': hourly_volume,
        'hourly_volatility': hourly_volatility,
        'early_morning_volume': avg_early_morning_vol
    }
...
def analyze_h4_continuation(df_4h, df_15m):
    ...
    06:00/07:00 Paris time level before moving up?
    """
    print("\n" + "="*70)
    ...
    for i in range(10, len(df_4h) - 5):
        ...
        if window['Higher_High'].sum() >= 2 and window['Higher_Low'].sum() >= 2:
            ...
            day_15m = df_15m[df_15m['Date_Paris'] == current_date]
            
            if len(day_15m) > 0:
                # Check 06:00-07:00 Paris time window
                test_window = day_15m[
                    ((day_15m['Hour_Paris'] == 6) | (day_15m['Hour_Paris'] == 7))
                ]
                ...
                if len(test_window) > 0:
                    ...
                    if tested:
                        results['tested_0600_0700'] += 1
                    
                    # Check if price moved up in London session
                    london = day_15m[(day_15m['Hour_Paris'] >= 8) & (day_15m['Hour_Paris'] < 13)]
                    
                    if len(london) > 0:
                        london_high = london['High'].max()
                        entry_price = test_window['Close'].iloc[-1] if len(test_window) > 0 else day_15m[day_15m['Hour_Paris'] == 8]['Open'].iloc[0] if len(day_15m[day_15m['Hour_Paris'] == 8]) > 0 else 0
    ...
    test_rate = (results['tested_0600_0700'] / results['bullish_h4_periods'] * 100) if results['bullish_h4_periods'] > 0 else 0
    success_after_test = (results['moved_up_after_test'] / results['tested_0600_0700'] * 100) if results['tested_0600_0700'] > 0 else 0
    
    print(f"\nBullish H4 Periods: {results['bullish_h4_periods']}")
    print(f"Times price tested 06:00-07:00 Paris level: {results['tested_0600_0700']} ({test_rate:.1f}%)")
...
def main():
    ...
    print("All times displayed in Paris time (CET/CEST, UTC+1/+2)")
    print("="*70)
    print()
...
def generate_summary_report(results):
    ...
    report.append("All times in Paris time (CET/CEST, UTC+1/+2)")
    report.append("="*70)
    report.append("")
    ...
    with open("/home/runner/work/Backtest-Trading/Backtest-Trading/nq_analysis_results_paris.txt", "w") as f:
        f.write(report_text)
