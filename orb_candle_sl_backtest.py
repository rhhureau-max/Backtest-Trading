...
CANDLE-BASED STOP LOSS VERSION

This script implements an ORB backtesting strategy with stop loss placed
under/above the breakout 5-minute candle, testing R:R ratios from 1 to 5.
...
class Trade:
    ...
    breakout_candle_low: float = 0.0
    breakout_candle_high: float = 0.0


class ORBCandleSLBacktester:
    """Opening Range Breakout Backtester with Candle-Based Stop Loss"""
    ...
    def find_breakout_with_candle(self, day_data: pd.DataFrame, range_info: dict) -> Optional[dict]:
        """Find the first breakout after the opening range and return candle info"""
    ...
    def calculate_candle_stops_and_targets(self, direction: str, entry_price: float, 
                                            breakout: dict) -> Optional[dict]:
        """Calculate stop loss based on breakout candle and take profit levels for RR 1-5"""
        if direction == 'LONG':
            # Stop loss under the low of breakout candle
            stop_loss = breakout['candle_low']
            ...
            # Stop loss above the high of breakout candle
            stop_loss = breakout['candle_high']
    ...
    def run_backtest(self) -> List[Trade]:
        ...
        for date in unique_dates:
            ...
            # Find breakout with candle info
            breakout = self.find_breakout_with_candle(day_data, range_info)
            if breakout is None:
                skipped_no_breakout += 1
                continue
            
            # Calculate candle-based stops and targets
            levels = self.calculate_candle_stops_and_targets(
                breakout['direction'],
            ...
            trade = Trade(
                ...
                breakout_candle_low=breakout['candle_low'],
                breakout_candle_high=breakout['candle_high']
            )
    ...
    def generate_report(self, all_metrics: dict) -> str:
        ...
        report.append("## Candle-Based Stop Loss with R:R Analysis (1:1 to 1:5)")
        report.append("")
        report.append("---")
        report.append("")
        ...
        report.append("with **candle-based stop loss** placement, testing multiple R:R targets from 1:1 to 1:5.")
        report.append("")
        ...
        report.append("- **Stop Loss**: Under the LOW of breakout candle (Long) / Above the HIGH (Short)")
        ...
        report.append("| Metric | Value |")
        report.append("|--------|-------|")
        report.append(f"| Average Range Size | {all_metrics['avg_range_size']:.2f} points |")
        ...
        report.append("## Notes")
        report.append("")
        report.append("- **Stop Loss Placement**: Under the LOW of the breakout 5-minute candle (for longs)")
        report.append("- **Risk Calculation**: Entry price minus Stop Loss price = Risk in points")
...
def main():
    ...
    print("=" * 70)
    print("Opening Range Breakout (ORB) Backtest - CANDLE-BASED STOP LOSS")
    ...
    backtester = ORBCandleSLBacktester(data_dir)
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("RESULTS SUMMARY - Candle-Based Stop Loss")
    print("=" * 70)
    print()
    print(f"Total Trades: {len(trades)}")
    print(f"Average Risk (SL Distance): {all_metrics['avg_risk']:.2f} points")
    print(f"Average Range Size: {all_metrics['avg_range_size']:.2f} points")
    print()
    print("-" * 70)
    ...
    report_path = data_dir / "ORB_Candle_SL_Report.md"
