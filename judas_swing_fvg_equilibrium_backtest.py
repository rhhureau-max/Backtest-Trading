...
class JudasSwingFVGEquilibriumBacktest:
    """Backtesting class with SL at swing extremity and TP at equilibrium"""
    ...
    def run_backtest(self) -> Dict:
        """
        Run the complete backtest with equilibrium TP/SL strategy
        ...
        for current_date in unique_dates:
            ...
            # Calculate equilibrium (midpoint between Asia high and low)
        ...
        avg_risk_reward = trades_df['tp_points'].mean() / trades_df['sl_points'].mean() if trades_df['sl_points'].mean() > 0 else 0
...
def run_backtest(csv_file: str) -> Dict:
    """
    Convenience function to run backtest with equilibrium strategy
    ...
    Returns:
        Dictionary with backtest results
    """
    backtest = JudasSwingFVGEquilibriumBacktest(csv_file)
...
def main():
    ...
    print("="*80)
    print("JUDAS SWING + FVG INVERSION - EQUILIBRIUM STRATEGY BACKTEST")
    print("="*80)
    print()
    print("Strategy Configuration:")
    print("  - SL: Placed at swing extremity (Asia_Low for LONG, Asia_High for SHORT)")
    print("  - TP: Placed at equilibrium (midpoint between Asia_High and Asia_Low)")
    ...
    if test_file:
        ...
        if results:
            print()
            print("="*80)
            print("EQUILIBRIUM STRATEGY RESULTS")
            print("="*80)
            print()
            ...
            print(f"Average Risk/Reward Ratio: {results['avg_risk_reward']:.2f}")
            print()
            print("="*80)
            ...
            if results['trades']:
                ...
                output_file = 'backtest_equilibrium_results.csv'
                trades_df.to_csv(output_file, index=False)
                print(f"\n✓ Detailed trade results saved to: {output_file}")
